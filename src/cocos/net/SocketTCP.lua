...
local SOCKET_CONNECT_FAIL_TIMEOUT = 8	-- socket failure timeout
...
local STATUS_SHUTDOWN = "Cannot send after socket shutdown"
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		while true do
    	    ...
    	    if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED or __status == STATUS_SHUTDOWN then
		    	self:close()
...
function SocketTCP:_stopreconnect()
